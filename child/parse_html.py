...
def parse_price(soup_price_block):
	'''
	...
	if len(soup_price_block) != 1:
		raise ValueError('The page has a different size on the price block')
...
def parse_attributes(attrs_block):
	'''
	Parse attributes
	:param attrs_block:
	:return:
	...
	if len(attrs_block) != 1:
		raise ValueError('The page has a different size on the attributes block')
...
def parse_additives(addts_block):
	'''
	...
	if len(addts_block) <= 1:
		raise ValueError('The page has a different size on the aditions block')
	audits_final_list = []
...
def parse_get_location(local_block):
	'''
	...
	if len(local_block) > 1:
		raise ValueError('The page has a different size on the local block')
